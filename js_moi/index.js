...
document.getElementById("tinhTien").onclick = function () {
  ...
  if (soKm <= 19) {
    ...
    tongTien =
      1 * tienKmDauTien + 18 * tienKmDauTien + (soKm - 19) * tiemKmTu19TroLen;
  }

  let soLanPhat = Math.floor((thoiGianCho - 3) / 3);
  tongTien += soLanPhat * tienTgCho;

  document.getElementById("thanhTien").innerHTML = `${tongTien.toLocaleString(
    "vi-VN"
  )} VND`;
...
document.getElementById("btnInHoaDon").onclick = function () {
  ...
  // thay đổi nội dung trong thẻ tbody( ... thành innerHTML)
